...
The pinned-tool workflow creates its commits through GitHub's `createCommitOnBranch` API and verifies their signatures before opening or refreshing a pull request. This satisfies the signed-commit requirement on `main`. Refreshing the bot branch uses an explicit lease so a concurrent branch update is not overwritten.
