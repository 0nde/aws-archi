...
Do not run the release assistant until the image publication workflow for the tag has completed successfully. The assistant never creates a tag, publishes a release, merges its pull request or moves an existing reference. A failed publication is fixed in source and released with a new version; an already published release tag is never rewritten.
